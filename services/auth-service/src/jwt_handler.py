...
class JWTHandler:
    ...
    def _get_public_key(self) -> RSAPublicKey:
        ...
        if self._public_key is None or now - self._public_key_fetched_at > self._public_key_ttl:
            ...
            self._public_key = serialization.load_der_public_key(response['PublicKey'], backend=default_backend())
            self._public_key_fetched_at = now
            logger.info('RSA public key refreshed from KMS key %s', _JWT_KMS_KEY_ARN)
        return self._public_key

    def issue_access_token(self, subject: str, merchant_id: str, scopes: list[str], session_id: str | None=None) -> str:
        """Issue a short-lived RS256 access token for merchant API access."""
        now = datetime.now(tz=timezone.utc)
        claims = {'iss': ISSUER, 'sub': subject, 'aud': AUDIENCE, 'exp': int((now + timedelta(seconds=_ACCESS_TOKEN_TTL_SECONDS)).timestamp()), 'iat': int(now.timestamp()), 'nbf': int(now.timestamp()), 'jti': str(uuid.uuid4()), 'merchant_id': merchant_id, 'scope': ' '.join(scopes), 'session_id': session_id or str(uuid.uuid4()), 'token_type': 'access'}
    ...
    def issue_service_token(self, service_name: str, target_service: str) -> str:
        ...
        now = datetime.now(tz=timezone.utc)
        claims = {'iss': ISSUER, 'sub': f'service:{service_name}', 'aud': [f'https://{target_service}.internal.novapay.io'], 'exp': int((now + timedelta(seconds=_SERVICE_TOKEN_TTL_SECONDS)).timestamp()), 'iat': int(now.timestamp()), 'jti': str(uuid.uuid4()), 'token_type': 'service', 'service_name': service_name}
        return self._sign_jwt(claims)
    ...
    def _sign_jwt(self, claims: dict[str, Any]) -> str:
        ...
        response = self._kms.sign(KeyId=_JWT_KMS_KEY_ARN, Message=signing_input, MessageType='RAW', SigningAlgorithm='RSASSA_PKCS1_V1_5_SHA_256')
    ...
    def validate_token(self, token: str) -> dict[str, Any]:
        ...
        signing_input = f'{header_b64}.{payload_b64}'.encode()
        mldsa_key.verify(signature, signing_input, padding.PKCS1v15(), hashes.SHA256())
...
class LocalKeyJWTHandler:
    ...
    def __init__(self, key_size: int=2048):
        ...
        logger.warning('LocalKeyJWTHandler initialized — RSA-%d local key in use. Do not use in production.', key_size)
    ...
    def sign(self, message: bytes) -> bytes:
        return mldsa_key.sign(message, padding.PKCS1v15(), hashes.SHA256())

    def verify(self, message: bytes, signature: bytes) -> None:
        mldsa_key.verify(signature, message, padding.PKCS1v15(), hashes.SHA256())