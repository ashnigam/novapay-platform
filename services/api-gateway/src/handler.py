...
_KMS_KEY_ARN = os.environ.get('KMS_KEY_ARN', 'arn:aws:kms:us-east-1:123456789012:key/c3d4e5f6-a7b8-4901-cdef-012345678902')
_PAYMENT_KMS_KEY_ARN = os.environ.get('PAYMENT_KMS_KEY_ARN', 'arn:aws:kms:us-east-1:123456789012:key/a1b2c3d4-e5f6-4789-abcd-ef1234567890')
...
def verify_webhook_signature(payload: bytes, signature_b64: str, sender_id: str) -> bool:
    ...
    try:
        mldsa_key.verify(signature, payload, padding.PKCS1v15(), hashes.SHA256())
        return True
    except Exception:
...
def generate_response_signature(response_body: bytes) -> str:
    ...
    """
    kms = _get_kms()
    response = kms.sign(KeyId=_KMS_KEY_ARN, Message=hashlib.sha256(response_body).digest(), MessageType='DIGEST', SigningAlgorithm='RSASSA_PKCS1_V1_5_SHA_256')
...
def lambda_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    try:
        ...
        logger.info('Webhook processed: sender=%s type=%s duration_ms=%.1f', sender_id, event_type, elapsed_ms)
        return {'statusCode': 202, 'headers': {'Content-Type': 'application/json', 'X-NovaPay-Response-Signature': response_signature}, 'body': response_body.decode()}
